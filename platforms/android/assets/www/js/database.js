...
function createDB() {
	...
	g_db.transaction( function (txn) {
		txn.executeSql('CREATE TABLE message(id integer NOT NULL PRIMARY KEY AUTOINCREMENT, content TEXT NOT NULL, pushtime DATETIME NOT NULL, read integer DEFAULT 0);', [], function(tx, res) {
...
//message: 插入的消息
//无返回结果
function insertMessage(message, insertMessageCallback) {
	...
	g_db.transaction( function (txn) {
		...
		txn.executeSql(sqlInsert, [], function(tx, res) {
		    ...
		    messageCnt = res.rows.length;
			console.log("sqlInsert res.rows.length is " + res.rows.length);
		 });
